...
new_user = models.User(email='test0@test0.com', full_name='test0', password=generate_password_hash('password', method='sha256'), role='student')
db.session.add(new_user)
db.session.commit()

new_user = models.User(email='test1@test1.com', full_name='test1', password=generate_password_hash('password', method='sha256'), role='student')
db.session.add(new_user)
db.session.commit()
